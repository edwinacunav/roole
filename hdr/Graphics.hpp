...
namespace Roole
{
  ...
  public:
    ...
    static void flush();
    //! Finishes all pending Roole drawing operations and executes the code in f in a clean
    //! OpenGL environment.
    ...
    //! Note: You may not call any Roole rendering functions from within the
    //! functor.
    ...
    //! \param image_flags Pass Roole::IF_RETRO if you do not want the resulting image to use
    //! interpolation when it is scaled or rotated.
    static Roole::Image render(int width, int height, const std::function<void ()>& f,
                              unsigned image_flags = 0);
    //! Records a macro and returns it as an Image.
    static Roole::Image record(int width, int height, const std::function<void ()>& f);
    ...
    //! image to simulate lines, or contribute a better draw_line to Roole.
