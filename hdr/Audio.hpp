//! \file Audio.hpp
//! Contains all the classes of Roole's audio system.
...
namespace Roole
{
  ...
  public:
    ...
    //! Avoid leaving samples paused for too long, as they will still occupy one of Roole's
    //! limited channels.
    void pause();
    void resume();
  ...
  public:
    ...
    Channel play(bool looping = false) const;
    ...
    //! \param pan Can be anything from -1.0 (left) to 1.0 (right).
    Channel play_pan(bool looping = false) const;
    //! \param volume Can be anything from 0.0 (silence) to 1.0 (full volume).
    double volume() const;
    void set_volume(double volume);
    void set_speed(double speed);
