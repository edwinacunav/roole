...
namespace Roole
{
  //! Convenient all-in-one class that serves as the foundation of a standard Roole application.
  //! Manages initialization of all of Roole's core components and provides timing functionality.
  ...
  public:
    ...
    bool is_disposed();
    bool need_cursor() const;
    void set_need_cursor(bool need);
    ...
    //! This method is only useful if you want to integrate Roole with another library that has
    //! its own main loop.
    ...
    //! or keep being shown. virtual bool tick();
    ...
    virtual void button_down(Roole::Button);
    virtual void press_button(Roole::Button) {}
    //! Same as button_down. Called when the user releases a button.
    virtual void button_up(Roole::Button) {}
