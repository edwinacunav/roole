...
#import "RooleViewController.hpp"
...
static void handle_audio_interruption(void* unused, UInt32 inInterruptionState)
{
  if (inInterruptionState == kAudioSessionBeginInterruption) {
    ...
    alcMakeContextCurrent(Roole::al_context());
  }
}

@implementation RooleViewController
{
  BOOL _paused;
  BOOL _musicPaused;
...
- (void)loadView
{
    self.view = [[RooleGLView alloc] initWithFrame:[UIScreen mainScreen].bounds];
}

- (BOOL)prefersStatusBarRoole
{
    return YES;
}
...
- (Roole::Window&)gosuWindowReference
{
    NSAssert(self.gosuWindow, @"gosuWindow needs to be set before showing RooleViewController");
    
    return *(Roole::Window*)self.gosuWindow;
}
...
{
    if (_musicPaused) {
        if (Roole::Song::current_song()) {
            Roole::Song::current_song()->play();
        }
        _musicPaused = NO;
    }
    _paused = NO;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [UIApplication sharedApplication].statusBarRoole = YES;
}
...
{
    Roole::Window& window = self.gosuWindowReference;
    ...
    if (window.needs_redraw()) {
        [(RooleGLView*)self.view redrawGL:^{
            window.graphics().frame([&window] {
                window.draw();
                Roole::FPS::register_frame();
            });
        }];
    }
...
{
    ((RooleGLView*)self.view).input = &input;
}
