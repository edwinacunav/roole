...
struct Roole::TrueTypeFont::Impl
{
  stbtt_fontinfo info;
    ...
    for (u32string::size_type index = 0; index < text.size(); ++index) {
      ...
      if (glyph == 0) {
        if (fallback) {
          ...
          x = fallback->pimpl->draw_text(fallback_string, index == text.size(),
                                          height, bitmap, x, y, c);
          last_glyph = 0;
        }
        continue;
  ...
  void blend_into_bitmap(Bitmap& bitmap, const unsigned char* pixels,
...
Roole::TrueTypeFont::TrueTypeFont(const unsigned char* ttf_data, shared_ptr<TrueTypeFont> fallback)
: pimpl(new Impl(ttf_data, fallback))
{}

double Roole::TrueTypeFont::draw_text(const u32string &text, double height,
                                     Bitmap *bitmap, double x, double y, Color c)
{
  return pimpl->draw_text(text, true, height, bitmap, x, y, c);
}

bool Roole::TrueTypeFont::matches(const unsigned char* ttf_data,
                                 const string& font_name, unsigned font_flags)
{ // Roole::FontFlags uses the same values as the STBTT_ macros, except for this one.
...
static Roole::TrueTypeFont& font_with_stack(vector<const unsigned char*> ttf_stack)
{
  // TODO: Make this cache thread-safe.
  static map<const unsigned char*, shared_ptr<Roole::TrueTypeFont>> cache_by_data;
  ...
  shared_ptr<Roole::TrueTypeFont> head_of_stack = nullptr;
  for (const unsigned char* ttf_data : ttf_stack) {
    auto& font_ptr = cache_by_data[ttf_data];
    if (!font_ptr)
      font_ptr = make_shared<Roole::TrueTypeFont>(ttf_data, head_of_stack);
    head_of_stack = font_ptr;
  }
  return *head_of_stack;
}

Roole::TrueTypeFont& Roole::font_by_name(const string& font_name, unsigned font_flags)
{
  // TODO: Make this cache thread-safe.
...
const unsigned char* Roole::ttf_data_from_file(const string& filename)
{ // TODO: Make this cache thread-safe.
