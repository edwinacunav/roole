...
void Roole::TexChunk::set_tex_info()
{
...
Roole::TexChunk::TexChunk(shared_ptr<Texture> texture, int x, int y, int w, int h, int padding)
: texture(move(texture)), x(x), y(y), w(w), h(h), padding(padding)
{
  set_tex_info();
}

Roole::TexChunk::TexChunk(const TexChunk& parent, int x, int y, int w, int h)
...
Roole::TexChunk::~TexChunk()
{
  texture->free(x - padding, y - padding, w + 2 * padding, h + 2 * padding);
}

void Roole::TexChunk::draw(double x1, double y1, Color c1, double x2, double y2, Color c2,
...
unique_ptr<Roole::ImageData> Roole::TexChunk::subimage(int x, int y, int width, int height) const
{
  return unique_ptr<Roole::ImageData>(new TexChunk(*this, x, y, width, height));
}

Roole::Bitmap Roole::TexChunk::to_bitmap() const
{
  return texture->to_bitmap(x, y, w, h);
}

void Roole::TexChunk::insert(const Bitmap& original_bitmap, int x, int y)
{
