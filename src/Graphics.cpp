  ...
  namespace
  {
    ...
    Graphics& current_graphics()
    {
      if (current_graphics_pointer == nullptr)
        throw logic_error("Roole::Graphics can only be drawn to while rendering");
...
double Roole::Graphics::global_scale = 1.0;

struct Roole::Graphics::Impl
{
...
Roole::Graphics::Graphics(unsigned phys_width, unsigned phys_height)
: pimpl(new Impl)
{
...
Roole::Graphics::~Graphics()
{
  if (current_graphics_pointer == this)
    current_graphics_pointer = nullptr;
}

unsigned Roole::Graphics::width() const
{
  return pimpl->virt_width;
}

unsigned Roole::Graphics::height() const
{
  return pimpl->virt_height;
}

void Roole::Graphics::set_resolution(unsigned virtual_width, unsigned virtual_height,
...
void Roole::Graphics::frame(const function<void ()>& f)
{
  if (current_graphics_pointer != nullptr)
    throw logic_error("Cannot nest calls to Roole::Graphics::begin()");
...
void Roole::Graphics::flush()
{
  current_queue().perform_draw_ops_and_code();
  current_queue().clear_queue();
}

void Roole::Graphics::gl(const function<void ()>& f)
{
...
void Roole::Graphics::gl(Roole::ZPos z, const function<void ()>& f)
{
#ifdef ROOLE_IS_OPENGLES
  throw logic_error("Custom OpenGL ES is not supported yet");
#else
...
void Roole::Graphics::clip_to(double x, double y, double width, double height,
...
Roole::Image Roole::Graphics::render(int width, int height, const function<void ()>& f,
...
Roole::Image Roole::Graphics::record(int width, int height, const function<void ()>& f)
{
...
void Roole::Graphics::transform(const Roole::Transform& transform, const function<void ()>& f)
{
...
void Roole::Graphics::draw_line(double x1, double y1, Color c1,
...
void Roole::Graphics::draw_triangle(double x1, double y1, Color c1, double x2, double y2, Color c2,
...
void Roole::Graphics::draw_quad(double x1, double y1, Color c1, double x2, double y2, Color c2,
...
void Roole::Graphics::draw_rect(double x, double y, double width, double height, Color c,
                              ZPos z, Roole::AlphaMode mode)
{
  draw_quad(x, y, c, x + width, y, c, x, y + height, c, x + width, y + height, c, z, mode);
}

void Roole::Graphics::schedule_draw_op(const Roole::DrawOp& op)
{
  current_queue().schedule_draw_op(op);
}

void Roole::Graphics::set_physical_resolution(unsigned phys_width, unsigned phys_height)
{
...
unique_ptr<Roole::ImageData> Roole::Graphics::create_image(const Bitmap& src,
  unsigned src_x, unsigned src_y, unsigned src_width, unsigned src_height, unsigned flags)
{
  ...
  if ((flags & IF_TILEABLE) == IF_TILEABLE &&
    ...
    if (!data)
      throw logic_error("Internal texture block allocation error");
    return data;
  }
