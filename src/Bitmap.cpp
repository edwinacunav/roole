...
unsigned Roole::Bitmap::width() const
{
  return w;
}

unsigned Roole::Bitmap::height() const
{
  return h;
}

void Roole::Bitmap::swap(Bitmap& other)
{
...
void Roole::Bitmap::resize(unsigned width, unsigned height, Color c)
{
...
void Roole::Bitmap::blend_pixel(unsigned x, unsigned y, Color c)
{
  if (c.alpha() == 0) return;
...
void Roole::Bitmap::insert(const Bitmap& source, int x, int y)
{
  insert(source, x, y, 0, 0, source.width(), source.height());
}

void Roole::Bitmap::insert(const Bitmap& source, int x, int y, unsigned src_x, unsigned src_y,
...
void Roole::apply_color_key(Bitmap& bitmap, Color key)
{
...
void Roole::unapply_color_key(Bitmap& bitmap, Color color)
{
  Color* p = bitmap.data();
...
void Roole::apply_border_flags(Bitmap& dest, const Bitmap& source, unsigned src_x, unsigned src_y,
