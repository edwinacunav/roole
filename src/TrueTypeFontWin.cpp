...
const unsigned char* Roole::ttf_data_by_name(const string& font_name, unsigned font_flags)
{
  ...
  LOGFONT logfont = {
    0, 0, 0, 0,
    (font_flags & Roole::FF_BOLD) ? FW_BOLD : FW_NORMAL,
    (font_flags & Roole::FF_ITALIC) ? TRUE : FALSE,
    (font_flags & Roole::FF_UNDERLINE) ? TRUE : FALSE,
  ...
  if (HFONT font = CreateFontIndirect(&logfont)) {
    if (HDC hdc = GetDC(0)) {
      ...
      if (ttf_buffer_size != GDI_ERROR) {
        auto buffer = make_shared<Roole::Buffer>();
