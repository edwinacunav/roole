...
static const unsigned char* ttf_data_of_default_sans_serif_font()
{
  ...
  if (match_result == FcResultMatch) {
      ...
      if (FcPatternGetString(pattern, FC_FILE, 0, &filename) == FcResultMatch) {
          Roole::log("Found the default sans-serif font: '%s'", filename);
          ttf_ptr = Roole::ttf_data_from_file(reinterpret_cast<char*>(filename));
      }
  }
  FcPatternDestroy(pattern);
  return ttf_ptr;
