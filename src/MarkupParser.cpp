...
unsigned Roole::MarkupParser::flags() const
{
  unsigned flags = 0;
  if (b > 0) flags |= Roole::FF_BOLD;
  if (i > 0) flags |= Roole::FF_ITALIC;
  if (u > 0) flags |= Roole::FF_UNDERLINE;
  return flags;
}

bool Roole::MarkupParser::match_and_skip(const char* chars, size_t length)
{
...
bool Roole::MarkupParser::parse_markup()
{ // Open and close bold text spans.
  ...
  if (match_and_skip("</c>")) {
    if (c.size() > 1)
      c.pop_back();
    else // If the user pops the color stack empty, go back to the default color.
      c[0] = Color::WHITE;
    return true;
  }
...
bool Roole::MarkupParser::parse_escape_entity()
{
...
void Roole::MarkupParser::add_current_substring()
{
...
void Roole::MarkupParser::add_composed_substring(u32string&& substring)
{
...
void Roole::MarkupParser::flush_to_consumer()
{
  if (!substrings.empty()) {
...
Roole::MarkupParser::MarkupParser(unsigned base_flags, bool split_words,
...
void Roole::MarkupParser::parse(const std::string& markup_string)
{
  markup = markup_string.data();
  const char* end_of_markup = markup_string.data() + markup_string.length();
  while (markup < end_of_markup) {
    if (*markup == '<' && parse_markup()) continue;
    if (*markup == '&' && parse_escape_entity()) continue;
