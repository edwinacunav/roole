...
Roole::Image::Image(const char* filename, unsigned flags)
{
  Bitmap bmp;
...
Roole::Image::Image(const char* filename, unsigned src_x, unsigned src_y,
...
Roole::Image::Image(const Bitmap& source, unsigned flags)
{
  Image(source, 0, 0, source.width(), source.height(), flags).data_.swap(data_);
}

Roole::Image::Image(const Bitmap& source, unsigned src_x, unsigned src_y,
                   unsigned src_width, unsigned src_height, unsigned flags)
: data_(Graphics::create_image(source, src_x, src_y, src_width, src_height, flags))
{}

Roole::Image::Image(unique_ptr<ImageData>&& data)
: data_(data.release())
{
  if (!data_) throw invalid_argument("Roole::Image cannot be initialized with nullptr");
}

unsigned Roole::Image::width() const
{
  return data_->width();
}

unsigned Roole::Image::height() const
{
  return data_->height();
}

void Roole::Image::draw(double x, double y, ZPos z, double scale_x, double scale_y, Color c,
...
void Roole::Image::draw_mod(double x, double y, ZPos z, double scale_x, double scale_y, Color c1,
...
void Roole::Image::draw_rot(double x, double y, ZPos z, double angle,
...
Roole::ImageData& Roole::Image::data() const
{
  return *data_;
}

vector<Roole::Image> Roole::load_tiles(const Bitmap& bmp, int tile_width, int tile_height,
                                     unsigned flags)
{
  int tiles_x, tiles_y;
  vector<Image> images;
...
vector<Roole::Image> Roole::load_tiles(const char* filename, int tile_width, int tile_height,
                                     unsigned flags)
{
  Bitmap bmp;
  load_image_file(bmp, filename);
