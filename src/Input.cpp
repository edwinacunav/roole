...
struct Roole::Input::Impl
{
  ...
  : input(input), window(window)
  {
    ...
    int num_gamepads = min<int>(Roole::NUM_GAMEPADS, SDL_NumJoysticks());
  ...
  #if SDL_VERSION_ATLEAST(2, 0, 4) && !defined(ROOLE_IS_X)
...
Roole::Input::Input(void* window)
: pimpl(new Impl(*this, (SDL_Window*)window))
{}

Roole::Input::~Input() {}

bool Roole::Input::feed_sdl_event(void* event)
{
...
string Roole::Input::id_to_char(Button btn)
{
  ...
  // Always return lower case to be consistent with previous versions of Roole.
...
void Roole::Input::set_mouse_factors(double scale_x, double scale_y,
