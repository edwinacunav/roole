...
#ifndef ROOLE_IS_IPHONE // iPhone definitions live in WindowUIKit.cpp

#include "Window.hpp"
#include "GraphicsImpl.hpp"
#include <SDL.h>

static SDL_DisplayMode display_mode(Roole::Window* window)
{
  ...
  int index = window ? SDL_GetWindowDisplayIndex(Roole::shared_window()) : 0;
  SDL_DisplayMode result;
  SDL_GetDesktopDisplayMode(index, &result);
  return result;
}

unsigned Roole::screen_width(Window* window)
{
  return display_mode(window).w;
}

unsigned Roole::screen_height(Window* window)
{
...
static NSSize max_window_size(Roole::Window* window)
{
  // Keep in sync with SDL_cocoawindow.m.
  auto style = NSTitledWindowMask|NSClosableWindowMask|NSMiniaturizableWindowMask;
  auto index = window ? SDL_GetWindowDisplayIndex(Roole::shared_window()) : 0;
...
unsigned Roole::available_width(Window* window)
{
  return max_window_size(window).width;
}

unsigned Roole::available_height(Window* window)
{
...
static SIZE max_window_size(Roole::Window* window)
{
  RECT work_area;
  if (window == nullptr) {
    ...
    SDL_GetWindowWMInfo(Roole::shared_window(), &wm_info);
...
unsigned Roole::available_width(Window* window)
{
  return static_cast<unsigned>(Roole::screen_width(window) * 0.9);
}

unsigned Roole::available_height(Window* window)
{
  return static_cast<unsigned>(Roole::screen_height(window) * 0.8);
}
#endif
