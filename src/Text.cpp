...
double Roole::text_width(const u32string& text,
                      const string& font_name, double font_height, unsigned font_flags)
{
  ...
  return font.draw_text(text, font_height, nullptr, 0, 0, Color::NONE);
}

double Roole::draw_text(Bitmap& bitmap, double x, double y, Color c, const u32string& text,
                      const string& font_name, double font_height, unsigned font_flags)
{
  if (font_height <= 0)              throw invalid_argument("font_height must be > 0");
  if (font_flags >= FF_COMBINATIONS) throw invalid_argument("Invalid font_flags");
...
Roole::Bitmap Roole::layout_text(const string& text, const string& font_name,
...
Roole::Bitmap Roole::layout_markup(const string& markup, const string& font_name,
                                double font_height, double line_spacing,
                                int width, Alignment align, unsigned font_flags)
{
  if (font_height <= 0)              throw invalid_argument("font_height must be > 0");
  ...
  if (width >= 0) {
    ...
    MarkupParser parser(font_flags, false, [&lines](vector<FormattedString>&& line) {
      // Remove trailing \n characters from each line to avoid errors from Roole::text_width().
