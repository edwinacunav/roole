...
Roole::Macro::Macro(DrawOpQueue& queue, int width, int height)
: pimpl(new Impl)
{
  pimpl->width = width;
  pimpl->height = height;
...
int Roole::Macro::width() const
{
  return pimpl->width;
}

int Roole::Macro::height() const
{
  return pimpl->height;
}

void Roole::Macro::draw(double x1, double y1, Color c1, double x2, double y2, Color c2,
  double x3, double y3, Color c3, double x4, double y4, Color c4, ZPos z, AlphaMode mode) const
{
  ...
  Roole::Graphics::gl(z, [=] { pimpl->draw_vertex_arrays(x1, y1, x2, y2, x3, y3, x4, y4); });
}

const Roole::GLTexInfo* Roole::Macro::gl_tex_info() const
{
  return nullptr;
}

Roole::Bitmap Roole::Macro::to_bitmap() const
{
  return Roole::Graphics::render(pimpl->width, pimpl->height, [this] {
...
unique_ptr<Roole::ImageData> Roole::Macro::subimage(int x, int y, int width, int height) const
{
  return unique_ptr<ImageData>();
}

void Roole::Macro::insert(const Bitmap& bitmap, int x, int y)
{
  throw logic_error("Roole::Macro cannot be updated with a Roole::Bitmap yet");
}
