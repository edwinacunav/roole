...
Roole::WordInfo::WordInfo(const string& font_name, double font_height, vector<FormattedString> parts)
{
  assert (!parts.empty());
  ...
  // Remove the trailing backspace character to avoid errors from Roole::text_width().
...
void Roole::TextBuilder::flush_current_line(EndOfLineReason reason)
{
...
void Roole::TextBuilder::allocate_next_line()
{
...
void Roole::TextBuilder::resize_to_allocated_lines()
{
...
Roole::TextBuilder::TextBuilder(const string& font_name, int font_height, int line_spacing,
...
Roole::Bitmap Roole::TextBuilder::move_into_bitmap()
{
...
void Roole::TextBuilder::feed_word(vector<FormattedString>&& word)
{
  ...
  if (current_line_width + new_word.width > result.width()) {
    ...
    if (new_word.is_whitespace) return;
