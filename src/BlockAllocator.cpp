...
struct Roole::BlockAllocator::Impl
{
...
Roole::BlockAllocator::BlockAllocator(unsigned width, unsigned height)
: pimpl(new Impl)
{
    pimpl->width = width;
    pimpl->height = height;
...
Roole::BlockAllocator::~BlockAllocator()
{}

unsigned Roole::BlockAllocator::width() const
{
  return pimpl->width;
}

unsigned Roole::BlockAllocator::height() const
{
  return pimpl->height;
}

bool Roole::BlockAllocator::alloc(unsigned a_width, unsigned a_height, Block& b)
{
...
void Roole::BlockAllocator::block(unsigned left, unsigned top, unsigned width, unsigned height)
{
  pimpl->blocks.emplace_back(left, top, width, height);
}

void Roole::BlockAllocator::free(unsigned left, unsigned top, unsigned width, unsigned height)
{
